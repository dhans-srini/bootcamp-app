...
public class ModuleController {
		...
		try {
			...
			if (CollectionUtils.isNotEmpty(modules)) {
				...
				return new ResponseEntity<>(
						ServiceResponseUtils.setHttpResponse(HttpStatus.NO_CONTENT.value(), "No records founds", null),
	...
	public ServiceResponse saveModule(@RequestBody ModuleVO moduleVO, HttpServletRequest request) {
		...
		try {
			...
			if (Objects.nonNull(module)) {
				serviceResponse = ServiceResponseUtils.dataResponse(ServiceConstants.DATA_INSERTION_SUCCESS_CODE,
						"Module saved successfully", module);
			} else {
				serviceResponse = ServiceResponseUtils.dataResponse(ServiceConstants.DATA_INSERTION_FAILED_CODE,
						"Save Module failed", null);
			}
	...
	public ServiceResponse deleteModule(@PathVariable("moduleId") Long moduleId, HttpServletRequest request) {
		ServiceResponse serviceResponse;
		try {
			Module module = new Module();
			module.setId(moduleId);
			moduleService.doDeleteModule(module);
			serviceResponse = ServiceResponseUtils.dataResponse(ServiceConstants.DATA_DELETION_SUCCESS_CODE,
					"Module deleted successfully", null);
			return serviceResponse;
		} catch (BusinessServiceException be) {
			logger.error(be.getMessage(), be);
			serviceResponse = ServiceResponseUtils.dataResponse(ServiceConstants.DATA_DELETION_FAILED_CODE,
					be.getMessage(), null);
			return serviceResponse;
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
